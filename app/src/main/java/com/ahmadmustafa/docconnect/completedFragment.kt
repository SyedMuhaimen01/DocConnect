...
class completedFragment : Fragment() {
            ...
            val appointmentsListener = object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        ...
                        appointment?.let {
                            // Filter appointments for the current user and status completed
                            if (it.patientId == currentUser.uid && it.status == "completed") {
                                appointments.add(it)
                            }
                        }
                    }
    ...
    private fun displayAppointmentsWithProfessionals(appointments: List<bookAppointment.Appointment>) {
        val adapter = CompletedAppointmentsAdapter(appointments)
